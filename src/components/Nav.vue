...
const profileLinks = [{ title: 'Following', link: '/FeedbackForum/account/following' }, { title: 'My Posts', link: '/FeedbackForum/account/posts' }]
const viewNotification = (notification) => {
  props.fetch('/api/account/notification', 'POST', notification)
  props.router.push(`/FeedbackForum/posts/${notification.post_id}`)
}
    ...
    <v-app-bar :elevation="2">
      <template v-slot:prepend>
        <v-btn prepend-icon="mdi-music" color="primary">
          <v-app-bar-title @click="props.router.push(`/FeedbackForum/`)">FeedbackForum</v-app-bar-title>
        </v-btn>
      </template>

      <v-btn append-icon="$plus" variant="outlined" @click="props.router.push(`/FeedbackForum/new`)">Post</v-btn>
